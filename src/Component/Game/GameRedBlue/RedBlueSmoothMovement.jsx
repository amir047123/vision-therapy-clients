...
function RedBlueSmoothMovement() {
  ...
  const [letterColor, setLetterColor] = useState("red"); // Initialize letter color as red
  ...
  const insertFunction = useCallback(() => {
    if (previousStateRef?.current?.length) {
      ...
      PostHooks(
        ...
        {
          gameName: "Smooth Movement Binocular",
          userId: userInfo?._id,
  ...
  const handleKeyPress = (event) => {
    ...
    if (gameActive) {
      ...
      if (pressedKey === movingLetter) {
        audio.play();
        setLetterColor((prevColor) => (prevColor === "red" ? "blue" : "red")); // Toggle between red and blue
  ...
  const textStyle = {
    color: letterColor,
    opacity: letterColor === "red" ? `${red}%` : `${blue}%`,
  ...
  return (
    <div
      className="flex fixed inset-0 items-center justify-center min-h-screen"
      style={gameContainerStyle} // Set the background color
    >
      {gameActive && (
        <div
      ...
      <div className="absolute top-0 right-0 p-4 text-xl">
...
export default RedBlueSmoothMovement;
