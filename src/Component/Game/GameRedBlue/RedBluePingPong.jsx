...
import hitSound from "../../../Assets/sound/PingPong/decidemp3-14575.mp3";
...
const RedBluePingPong = () => {
  ...
  useEffect(() => {
    ...
    const ball = {
      ...
      velocityY: 5,
      speed: 7,
      color: "blue", // Change "WHITE" to "BLUE"
      opacity: 1.0,
    };
    ...
    function drawArc(x, y, r, color, opacity) {
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.arc(x, y, r, 0, Math.PI * 2, true);
      ctx.closePath();
      ctx.fill();
      ctx.globalAlpha = ball?.color=="red"?red/100:blue/100;
    }
    ...
    function collision(b, p) {
      ...
      if (isUserPaddleCollision) {
        ...
        // Toggle the ball's color between "blue" and "red"
        // setBallColor((prevColor) => (prevColor === "blue" ? "red" : "blue"));

        // // Set the ball color based on the newColor state
        // ball.color = ballColor;
        // // Change the ball color to the next color in the array
      }

      if (isComputerPaddleCollision) {
    ...
    function update() {
      ...
      // Inside the collision logic, where the ball hits a paddle:
      let playerHit = collision(ball, player);
      let comHit = collision(ball, com);

      if (collision(ball, player)) {
        // Play sound and change the color only if the player's paddle is hit
        // hit.play();
        ball.color = ball.color === "blue" ? "red" : "blue";
        // Toggle the color between blue and red only if the player's paddle is hit
        // ball.opacity =
        //   ball.color === "blue"
        //     ? `${+blueRef.current / 100}`
        //     : `${+redRef.current / 100}`;
        // We check where the ball hits the player's paddle
        let collidePoint = ball.y - player.y - player.height / 2;
        // Normalize the value of collidePoint, we need to get numbers between -1 and 1.
        // -player.height/2 < collide Point < player.height/2
        collidePoint = collidePoint / (player.height / 2);

        // When the ball hits the top of the player's paddle, we want the ball to take a -45 degrees angle
        ...
        // Speed up the ball every time the player's paddle hits it
        ball.speed += 0.1;
      }
    ...
    function render() {
      ...
      // draw the ball
      drawArc(ball.x, ball.y, ball.radius, ball.color, ball.opacity);
    }
...
export default RedBluePingPong;
