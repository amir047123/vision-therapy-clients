...
const RedBlueSearchQuest = () => {
  ...
  const blueTextColor = "blue";
  const redTextColor = "red";
  const [textColorClass, setTextColorClass] = useState(blueTextColor); // Default to blue
  const suggestionBlueColor = "blue"; 
  const [suggestionColorClass, setSuggestionColorClass] = useState(
    suggestionBlueColor // Default to blue
  );
  ...
  const generateSuggestion = () => {
    ...
    // Set the suggestion letter's color based on the current table data color
    setSuggestionColorClass(textColorClass);
  };
  ...
  const insertFunction = useCallback(() => {
    if (previousStateRef?.current?.length) {
      ...
      PostHooks(
        ...
        {
          gameName: "Search Quest Binocular",
          userId: userInfo?._id,
        ...
        "searchQuest movement score updated !"
      );
    }
  ...
  const handleCellClick = (rowData) => {
    ...
    if (rowData === suggestionRef.current) {
      ...
      // Toggle the text color class between blue and red
      setTextColorClass((prevClass) =>
        prevClass === blueTextColor ? redTextColor : blueTextColor
      );
  ...
  return (
    <div
      className={`flex items-center justify-center inset-0 fixed bg-black ${
    ...
    >
      ...
      >
          ...
          {tableData.map((rowData, index) => (
            <div
              ...
              style={{
                ...
                opacity: textColorClass === "red" ? `${red}%` : `${blue}%`,
              }}
        ...
        <div
          style={{ color: suggestionColorClass }}
          className="border-t-4  w-full flex justify-between items-center"
        >
...
export default RedBlueSearchQuest;
